...
서버가 없다. Firebase가 백엔드 전체이고, **보안은 Firestore 규칙이 담당한다.**
...
**규칙 하나**: 컴포넌트가 Firebase SDK를 직접 부르지 않는다. 전부 `src/services/`를 지난다.
`src/lib/`은 반대로 Firebase를 모르는 순수 유틸만 담는다.

---

## 1. 데이터 모델과 보안 경계

핵심 개념은 **커플(couple)**이다. 두 사용자가 하나의 `couples/{id}` 문서를 공유하고,
그 아래 기록이 달린다.
...
`isCoupleMember(coupleId)`는 `couples/{id}.users` 배열에 요청자 uid가 있는지 확인한다.

> **초대 코드 전이 규칙이 핵심 방어선이다.** 참여자는 코드를 "소비"만 할 수 있고,
> 생성자로 위장하거나 다른 필드를 건드릴 수 없다(`affectedKeys().hasOnly`).
> 이 규칙들은 `npm run test:rules`(Firebase 에뮬레이터 + Vitest)로 테스트한다.
...
장소 추가 플로우(`pages/add/`)는 **국가 → 지역 → 장소 검색 → 핀 폼** 4단계다.
지역 정보가 먼저 확정되어야 지역 채우기 지도가 갱신되기 때문에 순서가 고정되어 있다.

---

## 3. 테마 시스템 — 프리셋 × 팔레트 × 모드

`ThemeContext`가 세 축을 독립적으로 관리한다.
...
**폰트 프리셋은 글꼴만 바꾸는 게 아니라 조형 언어 전체를 바꾼다.**
`<html>`의 `data-font` 속성 하나로 토큰이 통째로 갈린다.
...
> 이 설계의 대가로 **하드코딩된 색/모서리/그림자는 전부 금지**다.
> 하나라도 리터럴을 쓰면 sketch 프리셋에서 그 요소만 튄다.

---

## 4. 컴포넌트 계약

**"직접 만들지 말고 있는 걸 쓴다"**가 이 프로젝트의 UI 규칙이다.
...
버튼은 전부 `rounded-full`이다.

### 레이아웃 계약

앱 최대 폭은 **430px 중앙 정렬**(모바일 PWA)이다. 그래서 `fixed` 요소는
뷰포트가 아니라 **앱 폭**에 맞춰야 한다.
...
BottomNav는 인증된 모든 페이지에서 항상 보인다.

---

## 5. 마이크로 인터랙션 (`components/micro/`)

기록 앱에서 "쌓이는 느낌"을 만드는 작은 연출들을 컴포넌트로 분리했다.
...
`docs/PROJECT_OVERVIEW.md`는 **초기 기획 문서**다. 스택 설명(Next.js 14 App Router)이
현재 구현(React 19 + Vite)과 다르므로, 구조에 대한 현행 기준은
이 문서와 [`../README.md`](../README.md)다.
